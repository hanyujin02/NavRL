#!/bin/bash
# OOD generalization probe: same 12 NavRL checkpoints (all trained with
# env.obstacle_mode=hf — flat, axis-aligned 2.5D heightfield pillars), now
# evaluated CLEAN (no depth noise) on env.obstacle_mode=oriented — a single
# merged mesh of boxes with random yaw + roll/pitch tilt (see
# training/cfg/env.yaml and training/scripts/oriented_obstacles.py). None of
# these checkpoints ever saw a tilted obstacle during training; this measures
# how much each pretraining source's features generalize to obstacle
# geometry outside the training distribution, holding policy_3d=false
# (original 2D flight-height behavior, matching how these were trained —
# changing that too would confound the obstacle-geometry effect with a
# reward/behavior-regime change) and attitude_obs=true (already required by
# these checkpoints' 10-dim state input).
#
# env.obstacle.seed is left at its env.yaml default (2) for every run here
# (not overridden per-checkpoint, unlike the top-level RL `seed=`) so all 12
# checkpoints face the IDENTICAL oriented-obstacle field — apples-to-apples.
#
# eval.run_noise=false / eval.include_clean=true: only the clean condition
# runs (no dropout/cutout/etc. layered on top yet).
...
for idx in "${!RUNS[@]}"; do
  ...
  log_tag=$(echo "${wandb_name}_oriented3d_r25" | tr '/ ' '__')
  log_file="${LOG_DIR}/${log_tag}.log"

  echo "=============================================================="
  echo "[$((idx+1))/${N}] run=${run_dir}  step=${step}  seed=${seed}  wandb.name=${wandb_name}_oriented3d_r25"
  ...
  python training/scripts/eval.py --config-name eval_noise \
      ...
      wandb.name="\"${wandb_name}_oriented3d_r25\"" \
      > "$log_file" 2>&1
  status=$?
